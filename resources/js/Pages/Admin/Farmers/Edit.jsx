...
export default function Edit({ auth, farmer }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        // Pakai URL manual agar aman dari error Ziggy
        ...
        <AuthenticatedLayout
            ...
            header={
                <h2 className="text-3xl font-black text-emerald-950 italic uppercase tracking-tighter">
                    Edit Akun <span className="text-emerald-500">Petani</span>
                </h2>
            }
        >
            <Head title={`Edit Petani - ${farmer.name}`} />
            ...
            <div className="py-12 max-w-3xl mx-auto sm:px-6 lg:px-8">
                <div className="bg-white p-12 rounded-[3.5rem] border border-emerald-50 shadow-sm">
                    <form onSubmit={submit} className="space-y-6">
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Password Baru (Kosongkan jika tidak diganti)</label>
                            <input 
                                type="password" 
                                value={data.password} 
                        ...
                        <div className="flex gap-4 pt-6">
                            ...
                            <button 
                                disabled={processing} 
                                className="flex-1 bg-emerald-950 text-white py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-orange-500 transition shadow-xl"
                            >
                                {processing ? 'Menyimpan...' : 'Update Akun Petani'}
                            </button>
                        </div>
                    </form>
                </div>
